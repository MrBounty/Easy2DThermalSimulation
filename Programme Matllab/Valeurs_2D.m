...
if choix==1 || choix==0 || V(1)==0
    prompt = {'Nombre de point en x', 'Alpha (=0 expli, =1 impli)','Delta t [s]','Longueur en x [m]','rCp [J.m-3.K-1]','Lambda x[W.m-1.K-1]','Température de l air','Température initial','Nombre de point en y, si égale a 0 Ny=Nx','Largueur en y [m], si à 0 y=x','Lambda y[W.m-1.K-1], si à 0 égale à lambda x'};
    dlgtitle = 'Input';
    dims = [1 45];
...
if choix==3 || choix==0 || O(1)==2
    ...
    if O(6,1)==1
    prompt = {'puissance [W/m3]','Temps départ [s]','Temps final [s]','Point choisi en x, entre 1 et Nx','Point choisi en y, entre 1 et Ny'};
    ...
    if O(7,1)==1
    prompt = {'Température limite 1','Température limite 2','Température limite 3','Température limite 4'};
